...
| `LOGFLARE_ENABLE_POOL_CHECK`                   | Boolean, defaults to `true`                                             | Controls whether connection checks to Finch HTTP pools (BigQuery, Datadog, etc.) are executed. Set to `false` to disable pool checks in environments without external network connectivity, such as closed networks or testing environments.                          |
